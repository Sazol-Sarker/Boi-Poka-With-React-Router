  ...
  return (
    <div className="hero   md:bg-base-200 min-h-screen">
      ...
      <div className="hero-content flex-col lg:flex-row">
        <img
          src={image}
          className="w-96 h-96  max-w-[425px] max-h-[564px] rounded-lg shadow-2xl"
        />
        <div>
          <h1 className="text-2xl md:text-3xl lg:text-5xl font-bold mb-2">
            {bookName}
          </h1>
          <h1 className="text-lg md:text-xl ">By: {author}</h1>
          <div className="divider"></div>
          <p className="text-xl">
            {category}
            <sup>
              <span className="badge badge-outline ml-1 text-green-500 ">category</span>
            </sup>
          </p>
          ...
          <div className="flex  items-center my-2">
            ...
            {tags.map((tag, idx) => (
              <p key={idx} className="pl-8 text-green-500">
                #{tag}
              </p>
            ))}
          </div>
          ...
          <button
            onClick={() => addToLSDb(bookNo, "read-list")}
            className="btn btn-outline text-black font-bold text-lg mt-3"
          >
            Mark as Read
          </button>
          <button
            onClick={() => addToLSDb(bookNo, "wish-list")}
            className="btn btn-accent text-white font-bold text-lg ml-5"
          >
            Add to Wish List
          </button>
        </div>
